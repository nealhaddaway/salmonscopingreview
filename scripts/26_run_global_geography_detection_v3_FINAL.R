...
#          match and project-priority precedence.
# =============================================================================
...
mentions <- raw_mentions |>
  ...
  # If the same text maps to more than one gazetteer entity, retain only the
  # highest-priority mapping(s). This makes exact project overrides decisive,
  # e.g. New Brunswick -> Canada rather than the US city homonym.
  dplyr::group_by(
    record_sequence,
    source,
    token_start,
  ...
  dplyr::filter(
    priority == max(priority, na.rm = TRUE)
  ) |>
  dplyr::ungroup() |>
  dplyr::distinct(
    record_sequence,
