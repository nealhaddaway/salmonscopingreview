...
master <- read_corpus(include_file) |>
  dplyr::mutate(
    master_decision = "include"
  ) |>
  add_screening_keys()

message(sprintf(
  "Master relevance audit: loaded %d included records.",
  nrow(master)
))
...
message(sprintf(
  "Master relevance audit: model loaded with %d features.",
  length(fitted$model$features)
))

message("Master relevance audit: scoring master corpus.")
master$probability_relevant <- predict_relevance_probability(
  fitted$model,
  master
)

master$model_decision <- assign_screening_decision(
  master$probability_relevant,
  fitted$thresholds
)
...
# Distance from the model's validated exclusion boundary. Smaller values mean
# the record is closer to the model's exclusion region and therefore deserves
# more scrutiny among retained records.
master$distance_to_exclusion_threshold <-
  master$probability_relevant - fitted$thresholds$exclude_threshold

message("Master relevance audit: ranking records for manual review.")
review_queue <- master |>
  dplyr::filter(
    model_disagreement | model_uncertain
  ) |>
  dplyr::arrange(
    model_disagreement,
    distance_to_exclusion_threshold,
...
high_priority <- review_queue |>
  dplyr::filter(model_disagreement)

uncertain <- review_queue |>
  dplyr::filter(model_uncertain)

# A compact machine-readable audit retains all scored records so the audit is
# reproducible and the master itself remains untouched.
readr::write_csv(
  master |>
    dplyr::select(
      record_id,
  ...
  fs::path(output_dir, "master_relevance_scores.csv"),
  na = ""
)

readr::write_csv(
  high_priority,
  fs::path(output_dir, "high_priority_reconsideration.csv"),
  na = ""
)

readr::write_csv(
  uncertain,
  fs::path(output_dir, "uncertain_reconsideration.csv"),
  na = ""
)

summary <- tibble::tibble(
  item = c(
    "Included master records scored",
    "Automatic-exclude model recommendations",
    "Model-uncertain records",
    "Model supports inclusion",
    "Validated exclusion threshold",
    "Validated inclusion threshold"
  ),
  value = c(
    nrow(master),
    nrow(high_priority),
    nrow(uncertain),
    sum(master$model_decision == "automatic_retain"),
    fitted$thresholds$exclude_threshold,
    fitted$thresholds$include_threshold
  )
)

readr::write_csv(
  summary,
  fs::path(output_dir, "audit_summary.csv"),
  na = ""
)
...
wb$save(
  fs::path(output_dir, "master_relevance_audit.xlsx"),
  overwrite = TRUE
)
