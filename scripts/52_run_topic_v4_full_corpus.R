# =============================================================================
# File: scripts/52_run_topic_v4_full_corpus.R
# Purpose: Apply the frozen V4 topic classifier to all 12,074 corpus records.
#          The run checkpoints after every record and can be safely resumed.
# =============================================================================
...
checkpoint_file <- fs::path(
  output_dir,
  "topic_v4_full_corpus_checkpoint.rds"
)
...
failure_file <- fs::path(
  output_dir,
  "topic_v4_full_corpus_failures.csv"
)

system_prompt_file <- fs::path(
  output_dir,
  "topic_v4_full_corpus_system_prompt.txt"
)

ontology_prompt_file <- fs::path(
  output_dir,
  "topic_v4_full_corpus_ontology_prompt.txt"
)

progress_file <- fs::path(
  output_dir,
  "topic_v4_full_corpus_progress.csv"
)
...
records <- read_corpus(
  ...
  dplyr::transmute(
    sampling_stratum = "full_corpus",
    record_sequence = as.integer(record_sequence),
    record_id = as.character(record_id),
    title = dplyr::coalesce(as.character(title), ""),
    abstract = dplyr::coalesce(as.character(abstract), "")
  )

if (nrow(records) != 12074L) {
  warning(
    "Expected 12,074 corpus records but found ",
    nrow(records),
    ". Check the corpus file before continuing."
  )
}

if (anyDuplicated(records$record_id) > 0L) {
  stop("Duplicate record_id values found in the corpus.")
}
...
if (file.exists(checkpoint_file)) {
  ...
  message(
    "Resuming full-corpus V4 run from checkpoint: ",
  ...
  message("Starting final V4 topic classification of the full corpus.")
}
...
for (i in seq_len(nrow(remaining))) {
  ...
  progress <- tibble::tibble(
    completed = nrow(record_results),
    total = nrow(records),
    remaining = nrow(records) - nrow(record_results),
    failures = sum(record_results$classification_failed),
    review_required = sum(record_results$review_required, na.rm = TRUE),
    elapsed_minutes_this_session = elapsed_minutes,
...
message("")
message("Full-corpus V4 topic classification completed.")
...
message("")
message("Next step: draw a fresh 100-record validation sample from this run.")