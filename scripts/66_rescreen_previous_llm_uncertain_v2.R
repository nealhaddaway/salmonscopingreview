# =============================================================================
# File: scripts/66_rescreen_previous_llm_uncertain_v2.R
# Purpose: Re-screen only the records previously labelled uncertain by the LLM,
#          using the refined V2 prompt.
# =============================================================================
...
output_dir <- here::here(
  "outputs",
  "living_map_screening",
  "llm_screening_v2_test"
)

fs::dir_create(output_dir)

checkpoint_file <- fs::path(output_dir, "llm_screening_v2_test_checkpoint.rds")
record_output_file <- fs::path(output_dir, "llm_screening_v2_test_record.csv")
retain_file <- fs::path(output_dir, "llm_v2_retain.csv")
exclude_file <- fs::path(output_dir, "llm_v2_exclude.csv")
human_review_file <- fs::path(output_dir, "llm_v2_still_uncertain.csv")
failure_file <- fs::path(output_dir, "llm_v2_failures.csv")
prompt_file <- fs::path(output_dir, "llm_screening_v2_prompt.txt")
...
# The previous human-review file already contains the first-round LLM fields.
# Remove them before the V2 re-screen so the new results can join back without
# creating llm_decision.x / llm_decision.y suffixes.
records <- records_all |>
  dplyr::select(
    -dplyr::any_of(
      c(
...
for (i in seq_len(nrow(remaining))) {
  ...
  combined <- records |>
    ...
    dplyr::filter(!is.na(.data$llm_decision)) |>
