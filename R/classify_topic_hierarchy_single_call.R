# =============================================================================
# File: classify_topic_hierarchy_single_call.R
# Project: salmonscopingreview
# Purpose: Classify complete four-level topic paths in one LLM call
# =============================================================================

classify_topic_hierarchy_single_call <- function(
    title,
    abstract,
    ontology,
    model = "gpt-5-mini"
  ...
  system_prompt <- paste(
    ...
    "",
    "Semantic cues are non-exhaustive. They may be stems, abbreviations,",
    "examples, spelling variants or related concepts.",
    "Use them to understand each path's intended scope.",
    "Do not require an exact cue match.",
    "Do not assign a path from an isolated word alone.",
    "",
    "Do not assign paths mentioned only as background or motivation.",
    "Use only the listed hierarchy paths.",
    "Multiple paths are allowed when genuinely applicable.",
    "",
    ...
    "When mortality, morbidity, lesions, severity, prevalence, performance",
    "loss, disease resolution or treatment efficacy are measured, consider",
    "the relevant Pathology > Impacts path.",
    "",
    "Treat substances or interventions intended to stimulate immune function",
    "as pathology treatments when that is their aquaculture role.",
    "Treat probiotics, amino acids and functional ingredients as Feed >",
    "Additives when used as dietary interventions.",
    "Treat fillet or sensory quality outcomes as Consumption > Palatability",
    "when that path is available.",
    "",
    "Within the same broad-topic and subtopic branch, do not select",
    "General > General when a more specific parallel path is selected.",
    "Use General > General only when no more specific path in that branch fits.",
    "",
  ...
  body <- list(
    ...
    text = list(
      format = list(
        type = "json_schema",
        name = "topic_hierarchy_classification",
        strict = TRUE,
        schema = schema
      )
    )
  ...
  if (length(invalid_assignments) > 0L) {
    stop(
      "Invalid hierarchy paths returned: ",
