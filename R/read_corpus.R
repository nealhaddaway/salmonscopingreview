...
# Purpose: Parse and clean the RIS-like screened corpus
# =============================================================================
...
parse_ris_like <- function(path) {
  ...
  if (!file.exists(path)) {
    stop("Corpus file does not exist: ", path)
  }
  ...
  lines <- stringi::stri_enc_toutf8(
    lines,
    is_unknown_8bit = TRUE
  )
  
  end_idx <- which(
    stringr::str_detect(lines, "^ER\\s{2}-")
  )
  
  if (length(end_idx) == 0) {
    stop("No ER record terminators were found.")
  }
  ...
  parsed <- purrr::map2_dfr(
    ...
    function(start, end) {
      ...
      field_lines <- stringr::str_detect(
        block,
        "^[A-Z0-9]{2}\\s{2}-"
      )
