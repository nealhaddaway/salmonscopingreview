...
find_label_conflicts <- function(records) {
  title_conflicts <- records |>
    ...
    dplyr::filter(
      dplyr::n_distinct(eligibility) > 1L
    ) |>
    dplyr::ungroup() |>
    dplyr::mutate(
      conflict_basis = "exact normalised title"
    )
  ...
  doi_conflicts <- records |>
    ...
    dplyr::ungroup() |>
    dplyr::mutate(
      conflict_basis = "exact normalised DOI"
    )
...
collapse_training_duplicates <- function(records) {
  # Exact-title duplicates are grouped so equivalent records cannot be split
  # between training and validation. DOI alone is never used to collapse records.
  records |>
    dplyr::mutate(
...
fit_relevance_model <- function(training_records) {
  ...
  list(
    ...
    idf = as.numeric(
      log10(
        quanteda::ndoc(dfm_counts) /
          quanteda::docfreq(dfm_counts)
      )
    ),
...
transform_with_model <- function(text, model) {
  ...
  # Apply stored training-set IDF weights without learning from new records.
  x <- x %*% Matrix::Diagonal(
    x = model$idf
  )
...
predict_relevance_probability <- function(model, records) {
  x <- transform_with_model(
    records$screening_text,
    model
  )
...
deduplicate_new_records <- function(
  ...
  new <- new_records |>
    dplyr::select(
      -dplyr::any_of(dedup_output_columns)
    ) |>
    add_screening_keys() |>
    dplyr::mutate(
      incoming_row = dplyr::row_number()
    )
  ...
  # ---------------------------------------------------------------------------
  # 2. Exact DOI, but DOI is never sufficient by itself
  # ---------------------------------------------------------------------------
    ...
    doi_matches <- doi_matches |>
      dplyr::mutate(
        ...
        duplicate_basis = dplyr::case_when(
          ...
          TRUE ~
            "matching DOI but discordant titles"
        ),
  ...
  # ---------------------------------------------------------------------------
  # 3. Blocked fuzzy matching
  #
  # Candidate pairs are generated separately by year, first author, title
  # prefix and token key, then deduplicated. No Cartesian join is used.
  # ---------------------------------------------------------------------------
  ...
  candidate_year <- fuzzy_new |>
    ...
    dplyr::mutate(
      blocking_basis = "same year"
    )

  candidate_author <- fuzzy_new |>
    dplyr::filter(
      nzchar(first_author_key)
    ) |>
    dplyr::inner_join(
      fuzzy_master |>
        dplyr::filter(
          nzchar(master_first_author)
        ),
      by = c(
        "first_author_key" = "master_first_author"
      ),
      relationship = "many-to-many"
    ) |>
    dplyr::mutate(
  ...
  candidate_prefix <- fuzzy_new |>
    ...
    dplyr::inner_join(
      fuzzy_master |>
        dplyr::filter(
          nzchar(master_prefix)
        ),
      by = c(
        "title_prefix" = "master_prefix"
      ),
      relationship = "many-to-many"
    ) |>
    dplyr::mutate(
  ...
  candidate_tokens <- fuzzy_new |>
    dplyr::filter(
      nzchar(title_token_key)
    ) |>
    dplyr::inner_join(
      fuzzy_master |>
        dplyr::filter(
          nzchar(master_token_key)
        ),
      by = c(
        "title_token_key" = "master_token_key"
      ),
      relationship = "many-to-many"
    ) |>
    dplyr::mutate(
      ...
      dplyr::mutate(
        ...
        duplicate_basis = dplyr::case_when(
          duplicate_status == "probable_duplicate" ~
            paste0(
              "very high title similarity plus ",
              blocking_basis
            ),
          TRUE ~
            paste0(
              "high title similarity plus ",
              blocking_basis
            )
        ),
        match_priority = dplyr::case_when(
  ...
  # ---------------------------------------------------------------------------
  # 4. Choose the strongest available match for each incoming record
  # ---------------------------------------------------------------------------
  ...
  new |>
    ...
    dplyr::mutate(
      duplicate_status = dplyr::coalesce(
        duplicate_status,
        "new"
      ),
      duplicate_basis = dplyr::coalesce(
        duplicate_basis,
        ""
      )
    )
